...
public class AccountSqlDAO implements AccountDAO {
	...
	public void save(Account newAccount) {
		...
		jdbcTemplate.update(sqlInsertAccount, newAccount.getId(), 
	...
	public Account findAccountById(long id) {
		...
		String sqlFindAccountById = "SELECT * " +
									" FROM accounts " +
									" WHERE account_id = ?";
	...
	public List<Account> findAccountsByUserId(int userId) {
		...
		String sqlFindAccountByUserId = "SELECT * " +
										" FROM accounts " +
										" WHERE user_id = ?";
	...
	public Double findBalanceByAccountId(long accountId) {
		String sqlFindBalanceByAccountId = "SELECT balance " +
										   " FROM accounts " +
					 					   " WHERE account_id = ?";
	...
	public Double increaseBalance(Account account, Double amountToTransfer) {
		...
		Double initialBalance = jdbcTemplate.queryForObject(sqlSelect, Double.class, account.getId());
	...
	public Double decreaseBalance(Account account, Double amountToTransfer) {
		...
		Double initialBalance = jdbcTemplate.queryForObject(sql, Double.class, account.getId());
		Double updatedBalance = initialBalance - amountToTransfer;
		if (updatedBalance < 0) {
